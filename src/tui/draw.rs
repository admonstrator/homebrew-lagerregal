...
pub(super) fn draw_help_overlay(f: &mut Frame) {
    let lines = vec![
        ...
        Line::from("Enter, double-click   Launch it - open the app (cask) or run"),
        Line::from("                      its binary (formula, from the Cellar)"),
        Line::from("m, right-click   Open the action menu (incl. uninstall)"),
