...
The word-boundary rule only guards the *left* edge - keywords may still grow to the right, so a short keyword swallows longer words that start with it. `tex` would match "text editor", which is why the taxonomy spells out `latex`/`texlive` instead. Wrapping a short word in spaces (`" ai "`, `" ide "`) makes it exact in both directions: the leading space isn't alphanumeric, so the boundary rule is skipped, and the trailing space blocks "aim"/"identifier".

Heuristic *order* carries as much meaning as the keywords. Three placements are load-bearing: `Virtualization` sits above `Games & Emulation` (so qemu's "machine emulator and virtualizer" is a hypervisor, not a console emulator), `Version & Build Tools` above `Dev Tools & Languages` (version/build/dependency managers are a large family that would otherwise swell that category), and `Libraries` runs **last**, immediately before `Uncategorized` - "library" matches well over a thousand formulae, so it must only ever see what no category with something specific to say has already claimed (a DNS library is DNS, a graphics library is Media & Graphics). Adding a category means adding it to `theme.rs`'s `CATEGORY_STYLES` and its `BUILT_IN` list too; a test fails otherwise.
...
The formula is **not** in this repo. `.github/workflows/release.yml` builds four targets on tag push - `aarch64-apple-darwin`, `x86_64-apple-darwin`, `aarch64-unknown-linux-musl`, `x86_64-unknown-linux-musl` - uploads a tarball each to the GitHub release, then regenerates `Formula/lagerregal.rb` and pushes it to `main` of [`admonstrator/homebrew-tap`](https://github.com/admonstrator/homebrew-tap), the shared tap for everything distributed via `brew` here. That cross-repo push needs a token beyond the default `GITHUB_TOKEN` (which only has permissions in this repo): `GH_TAP_PAT`, a fine-grained PAT scoped to `contents: write` on `homebrew-tap` only, stored as a secret on this repo. `brew tap admonstrator/tap` is what users run; `homebrew-tap` may end up holding formulae for other projects too, so don't assume this repo's name maps 1:1 to the tap name. CI (`ci.yml`) runs fmt/clippy/tests once per target, each on a runner of that target's own architecture. Releasing = bump `Cargo.toml` version, commit, `git tag vX.Y.Z && git push --tags`.
